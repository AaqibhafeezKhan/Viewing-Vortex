...
`;
    ...
    formatRuntime(minutes) {
        if (typeof minutes !== 'number' || minutes < 1) return 'N/A mins';
    ...
    async getTrailerButton(data) {
        ...
        try {
            const
                response = await fetch(`https://api.themoviedb.org/3/${this.type}/${data.id}/videos?api_key=${TMDB_API_KEY}`);
            const
    ...
    saveToFavorites() {
        ...
        const exists = favorites.some(fav =>
            ...
            (!fav.data.id && fav.data.title === favoriteItem.data.title && fav.type === favoriteItem.type)
        );
